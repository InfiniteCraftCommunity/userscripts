...
(function(){
    ...
    async function buildAutoIncrement(base,tool,regex)
  {
    ...
    do{
       ...
       resultText= await mockCraft({emoji:inputElement.emoji,text:inputElement.text,top:window.innerHeight/2,left:left,height:41,width:100},
                       {emoji:toolElement.emoji,text:toolElement.text,top:window.innerHeight/2,left:left,height:41,width:100})

      if(!resultText)
          break;
       if(found.includes(resultText))
          break;
  ...
  function makeStartModal(){
    ...
    startButton.addEventListener("click",()=>{
                                 ...
                                 modal.close();
                                 if(baseInput!="" && toolInput!="" && regexBaseText!="")
                                 buildAutoIncrement(baseInput,toolInput,regexBaseText)
    ...
    inputDiv.appendChild(inputTitle)
     ...
     regexInput.addEventListener("focus",()=>{
       ...
       regexInput.addEventListener("mouseout",()=>{ console.log("mouse leave")
        inFocus=null;
      });
