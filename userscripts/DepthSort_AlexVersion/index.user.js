...
function computeDepth(elemIn)
{
    ...
    while (elemStack.length > 0) {
        ...
        if ( !isClosed) {
            ...
            if ( !isInSave || save["recipes"][elem].length === 0) { // element has no recipe
                elemStack.pop();
                recipeIdStack.pop();
                depthStack.pop();
                stackLen -= 1;
        ...
        if (isClosed || goneThroughAllRecipes) { // parent recipe has been gone through
            elemStack.pop();
            recipeIdStack.pop();
            depthStack.pop();
            stackLen -= 1;
            ...
            if (leftDepth === 0) { // is left (first) ingredient of the recipe
            ...
            } else { // parentDepth is always > 0, visitedParent too
        ...
        if (recipe[0].text === elem || recipe[1].text === elem) { // directly recursive recipe
            ...
            (recipe[0].text in visited && !closed.has(recipe[0].text))
            || (recipe[1].text in visited ) && !(closed.has(recipe[1].text)))
        {
            recipeIdStack[stackLen - 1] += 1;
            continue;
        }
  ...
  console.log("Elem in enter:",elem);
        ...
        if(recipeIdStack[stackLen - 1]>=save["recipes"][elem].length)
            {   console.log("no more recipes")
              if(visited[elem]>=Infinity || visited[elem]<=-Infinity)
                visited[elem]=Infinity;
              closed.add(elem);
              elemStack.pop();
              recipeIdStack.pop();
        ...
        {   console.log("Good Parents :",recipe[0].text,recipe[1].text)
            if(visited[elem]==undefined || visited[elem]<=-Infinity)
          visited[elem]= Math.max(visited[recipe[0].text],visited[recipe[1].text])+1;
           else
          visited[elem]= Math.min(visited[elem], Math.max(visited[recipe[0].text],visited[recipe[1].text])+1);
