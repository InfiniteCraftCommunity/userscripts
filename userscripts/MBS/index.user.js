...
// @downloadURL     https://github.com/InfiniteCraftCommunity/userscripts/raw/master/userscripts/MBS/index.user.js
// @updateURL       https://github.com/InfiniteCraftCommunity/userscripts/raw/master/userscripts/MBS/index.user.js
// ==/UserScript==
...
.particles, .pinwheel {
                ...
                for (const node of mutation.addedNodes) {
                    ...
                    if (!tooltips.length) {
                        for (const handler of tooltipHandlers.sort((a, b) => a.priority - b.priority)) {
                            ...
                            if (typeof handler.condition === "function" && handler.condition(element)) handler.handle(element, tooltips);
                        }
    ...
    function observeInstances({ instances }) {
    ...
    function makePannable({ container, instances, sidebar }) {
    ...
    function initRecentElements(elements) {
        ...
        if (recentElements.length === 0) recentContainer.style.display = "none";
    ...
    function updateRecentElements() {
        ...
        recentContainer.appendChild(recentTitle);
        if (recentElements.length > 0) recentContainer.style.display = "block";
        ...
        unsafeWindow.$nuxt.$root.$children[1].$children[0].$children[0].getCraftResponse = exportFunction(
            (...args) =>
                new window.Promise(async (resolve) => {
                    ...
                    if (first.text === "" || second.text === "" || result.text === "" || result.text === "Nothing") return resolve(response);
    ...
    function logContainerHeights({ sidebar, pinnedContainer }) {
    ...
    function loadSettings(elements) {
        ...
        for (const [id, config] of Object.entries(settings.tooltipHandlers)) {
            Object.assign(
                tooltipHandlers.find((x) => x.id === id),
                config
            );
        }
    ...
    for (const handler of tooltipHandlers)
