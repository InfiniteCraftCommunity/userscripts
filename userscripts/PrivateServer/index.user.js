...
(function () {
  ...
  function init$1() {
    _unsafeWindow.$nuxt.$root.$children[1].$children[0].$children[0].getCraftResponse = exportFunction(
      (...args) => new Promise(async (resolve) => {
        const recipe = [args[0], args[1]].sort(
          (a, b) => a.text.localeCompare(b.text)
        );
        const controller = new AbortController();
        let url;
        try {
          const urlInput = await _GM.getValue("ps-url") || "https://neal.fun/api/infinite-craft/pair?first={first}&second={second}";
          if (!/.*:\/\/.*={first}.*={second}/.test(urlInput)) {
            throw new PrivateServerError("Invalid URL");
          }
          url = urlInput.replace("{first}", encodeURIComponent(recipe[0].text)).replace("{second}", encodeURIComponent(recipe[1].text));
  ...
  async function initServerModal() {
    ...
    input.placeholder = "https://neal.fun/api/infinite-craft/pair?first={first}&second={second}";
    ...
    (_a = document.querySelector(".container")) == null ? void 0 : _a.append(modal);
    _modal = modal;
  }
  async function init() {
    var _a;
    ...
    (_a = document.querySelector(".side-controls")) == null ? void 0 : _a.prepend(serverButton);
