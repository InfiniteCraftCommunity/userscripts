...
let indexJ=0;
  ...
  function makeOneScreenshot(goToNext=null)
  {
    ...
    console.log(html2canvas(copy,{canvas:initialCanvas,backgroundColor:null

                                 }).then((canvas) => {
      copy.style.backgroundColor=keepBack;
           if(goToNext==null)
          adjustingAllInstances(1)
            else
          if(goToNext<3)
          adjustingAllInstances(goToNext+1)
...
//                  }
  ...
  function makeSetBasisModal()
  {
    ...
    let closeButton=document.createElement("button");
    closeButton.addEventListener("click",()=>{ base=saveBase;



                                              redrawBase(basisDiv); settingsModal.close();
    ...
    let saveButton=document.createElement("button");
     saveButton.addEventListener("click",()=>{settingsModal.close()})
...
function startingModal(){
    if(document.querySelector(".choose-target-lineage")!=null)
      return;
  else{
  set=false;
